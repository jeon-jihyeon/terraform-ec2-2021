...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name        = "${var.environment}-public-subnet-${format("%03d", count.index+1)}"
    Environment = var.environment
  }
}
...
resource "aws_route_table" "public" {
  ...
  tags = {
    Name        = "${var.environment}-rt-public-${format("%03d", count.index+1)}"
    Environment = var.environment
  }
}
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name        = "${var.environment}-private-subnet-${format("%03d", count.index+1)}"
    Environment = var.environment
  }
}
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name        = "${var.environment}-rt-private-${format("%03d", count.index+1)}"
    Environment = var.environment
  }
}
...
resource "aws_route" "private" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  instance_id            = element(var.bastion_nat_ids, count.index)
}
